...
| 4 | `contentforge:scientific-validator` | `phase-3-draft.md`, `phase-3.5-visual-manifest.json`, `phase-2-factcheck.md` | `phase-4-validation.md` + `phase-4-fixes.json` (the fix ledger — **required whenever the report names a correction**, empty `items` is valid) | **Gate 4:** zero hallucinations; every claim traceable to a cited source; logic consistent; **`fix-ledger.py validate` exits 0 and every correction in the report appears in the ledger** | Phase 3 (with the specific claims to fix) |
| 5 | `contentforge:structurer-proofreader` | `phase-3-draft.md`, `phase-4-validation.md`, `phase-4-fixes.json`, brand profile | `phase-5-structured.md` | **Gate 5:** **`fix-ledger.py verify` reports zero unresolved blocking items (HARD)**; zero grammar/spelling errors on re-scan; readability within ±0.5 grade of the content-type target (`text-metrics.py`); brand terminology compliance | Re-run Phase 5 |
