...
- **Every generated image requires user approval before embedding** — approval is collected by the ORCHESTRATOR after this phase returns (this agent records `approved_by_user: false`; Phase 8 embeds only approved images)
...
- Set `approved_by_user` to `false` — the orchestrator flips this to `true` only after the user approves; Phase 8 embeds ONLY user-approved AI images
...
- [ ] AI-generated images (if any) are recorded in the manifest with `ai_generated: true` and `approved_by_user: false` (orchestrator-level user approval pending; Phase 8 embeds only approved images)
