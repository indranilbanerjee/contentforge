...
Token Usage (estimated):
  Agent instructions: ~{instruction_tokens} tokens
  Content processing: ~{content_tokens} tokens
  Total estimated: ~{total_tokens} tokens

Guardrails: {guardrails_status}
Loops Used: {loop_count}
```
...
| AF | Token Estimate | Estimated total tokens from pipeline-tracker.py |
