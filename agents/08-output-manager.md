...
If Phase 3.5 generated AI images (check `manifest.json` for assets with `ai_generated: true`):

1. **Feature Image:** If `feature-image.png` exists in assets/:
   - Embed at the top of the document (before the first heading or as a cover image)
   - Set width to page width (6.5 inches at 150 DPI)
   - Add alt text from manifest

2. **Contextual Images:** For each AI-generated image (image-01.png, image-02.png, etc.):
   - Embed at the placement position specified in manifest
   - Set appropriate width (maintain aspect ratio, max 6.5 inches wide)
   - Add caption and alt text from manifest
   - Add "Image: AI-generated" attribution note in small italic text below caption

3. **Transparency:** All AI-generated images must include a small attribution:
   - In .docx: italic text below image: "Image generated by AI"
   - In markdown/HTML: `<small><em>Image generated by AI</em></small>`

**Visual asset count for completion summary:**
- Track total embedded charts vs TODO boxes vs AI-generated images
...
Parse the JSON output — use it to populate the PIPELINE PERFORMANCE and PIPELINE COMPLEXITY sections in the completion summary below.
...
**Phase Timing Columns (from pipeline-tracker.py report):**

| Column | Header | Value |
|--------|--------|-------|
| U | Processing Time (min) | Total pipeline time from pipeline-run.json |
| V | Phase 1 Time (s) | Research phase duration |
| W | Phase 2 Time (s) | Fact-check phase duration |
| X | Phase 3 Time (s) | Drafting phase duration |
| Y | Phase 3.5 Time (s) | Visual asset annotation duration |
| Z | Phase 4 Time (s) | Scientific validation duration |
| AA | Phase 5 Time (s) | Structuring/proofreading duration |
| AB | Phase 6 Time (s) | SEO/GEO optimization duration |
| AC | Phase 6.5 Time (s) | Humanization duration |
| AD | Phase 7 Time (s) | Review duration |
| AE | Phase 8 Time (s) | Output generation duration |
| AF | Content Words | Final word count of content piece |
| AG | Guardrails Status | "verified" / "skipped_empty" / "minimal" |

Load timing data from `pipeline-run.json` (generated by pipeline-tracker.py). If timing data is unavailable, leave columns blank.
