...
- Critical README sections (Supported surfaces, Quick start, Troubleshooting,
  FAQ, Release notes) go missing
"""
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def test_grok_marketplace_entry_matches_canonical_version(self):
        ...
        self.assertEqual(entry["name"], "contentforge")
    ...
    def test_readme_supported_surfaces_section_mentions_canonical(self):
        text = README.read_text(encoding="utf-8")
        # CF README uses "## Supported surfaces (v3.15.0)" pattern — keeps the section
...
class TestDescriptionConsistency(unittest.TestCase):
    ...
    def test_description_mentions_canonical_skill_count(self):
        # The CF description string contains "21 skills" — keep it true.
...
class TestInstallCommandCoverage(unittest.TestCase):
    ...
    def test_cursor_install_command_present(self):
        self.assertIn("/add-plugin contentforge", self.text)

    def test_copilot_install_command_present(self):
        self.assertIn("copilot plugin install contentforge", self.text)
...
class TestCriticalReadmeSections(unittest.TestCase):
    ...
    def test_why_contentforge_section(self):
        self.assertIn("## Why ContentForge", self.text)
    ...
    def test_what_contentforge_does_section(self):
        self.assertIn("## What ContentForge does", self.text)
    ...
    def test_updating_section(self):
        self.assertIn("## Updating", self.text)

    def test_faq_section(self):
        self.assertIn("## FAQ", self.text)

    def test_cross_platform_compatibility_section(self):
        self.assertIn("## Cross-platform compatibility", self.text)

    def test_release_notes_section(self):
        self.assertIn("## Release notes", self.text)

    def test_troubleshooting_covers_all_native_platforms(self):
        # Each native platform should appear by name somewhere in the troubleshooting
        # / cross-platform-compatibility / supported-surfaces region. (9 native
        # platforms since v4.1.0 added Grok.)
